...
def expected_offspring(couples):
    # Probabilities of producing an offspring with a dominant phenotype for each genotype pairing.
    probs = [1.0, 1.0, 1.0, 0.75, 0.5, 0.0]

    # Multiply the number of couples by the probability of producing an offspring with a dominant phenotype
    # for each genotype pairing, and sum these products.
    expected = sum([couples[i] * probs[i] for i in range(6)]) * 2

    return expected

if __name__ == '__main__':
    couples = list(map(int, input().strip().split()))
